...
const buildEmbed = (logs, pagina, total, objetivo) => {
  ...
  const embed = new EmbedBuilder()
    .setTitle('🔨 Logs de Baneos')
    .setDescription(
      objetivo
        ? `Baneos de **${objetivo.username}** — ${logs.length} en total`
        : `Todos los baneos del servidor — ${logs.length} en total`
    )
    .setColor(0xC0392B)
...
module.exports = {
  name: 'banlogs',
  help: {
    purpose: 'Muestra logs de baneos del servidor con paginación.',
    category: '📋 Logs',
    adminOnly: true,
  },
  async execute(message, args) {
    ...
    const logs = todos
      .filter(l => l.tipo === 'ban' && l.servidorId === message.guild.id)
      .reverse();

    if (!logs.length)
      return message.reply('📭 No hay logs de baneos en los últimos 30 días.');
    ...
    if (objetivo && !filtrados.length)
      return message.reply(`📭 No hay logs de baneos para ese usuario en los últimos 30 días.`);
