...
const buildEmbed = (logs, pagina, total, objetivo) => {
  ...
  const embed = new EmbedBuilder()
    .setTitle('👢 Logs de Kicks')
    .setDescription(
      objetivo
        ? `Kicks de **${objetivo.username}** — ${logs.length} en total`
        : `Todos los kicks del servidor — ${logs.length} en total`
    )
    .setColor(0xE74C3C)
...
module.exports = {
  name: 'kicklogs',
  help: {
    purpose: 'Muestra logs de kicks del servidor con paginación.',
    category: '📋 Logs',
    adminOnly: true,
  },
  async execute(message, args) {
    ...
    const logs = todos
      .filter(l => l.tipo === 'kick' && l.servidorId === message.guild.id)
      .reverse();

    if (!logs.length)
      return message.reply('📭 No hay logs de kicks en los últimos 30 días.');
    ...
    if (objetivo && !filtrados.length)
      return message.reply(`📭 No hay logs de kicks para ese usuario en los últimos 30 días.`);
